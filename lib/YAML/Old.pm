...
field dumper_class => 'YAML::Old::Dumper';
field loader_class => 'YAML::Old::Loader';
field dumper_object =>
...
sub DumpFile {
    ...
    if (ref $filename eq 'GLOB') {
        ...
        open $OUT, $mode, $filename
          or YAML::Old::Base->die('YAML_DUMP_ERR_FILE_OUTPUT', $filename, $!);
    }  
...
sub LoadFile {
    ...
    if (ref $filename eq 'GLOB') {
        ...
        open $IN, $filename
          or YAML::Old::Base->die('YAML_LOAD_ERR_FILE_INPUT', $filename, $!);
    }
...
YAML::Old is the old Perl YAML module. The old YAML.pm that was on CPAN
from the beginning of 2002 to the end of 2008. YAML.pm has been made
...
The difference is that now (the new) YAML.pm will search for the best
YAML implementation module available on your system. See L<YAML::Any>.
If you really need the behavior of the old YAML.pm, do this:
...
YAML.pm to YAML::Old. Later I'll revamp it in more detail.

=head1 DESCRIPTION

The YAML::Old module implements a YAML Loader and Dumper based on the YAML
...
YAML::Old to be plugged directly into modules like POE.pm, that use the
...
a copy of the original). Setting $YAML::Old::UseAliases to 0 will allow you
...
The YAML::Old distribution ships with a script called 'ysh', the YAML shell.
...
BIGGER WARNING: YAML::Old has been slow in the making, but I am committed
to having top notch YAML tools in the Perl world. The YAML team is close
to finalizing the YAML 1.1 spec. This version of YAML::Old is based off of
